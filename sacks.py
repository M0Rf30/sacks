...
import translations.translations_rc 
...
#definizione jabber con le sue connessioni
	...
	def connJabber(self,  connection):
		...
		if connection:
			...
			if self.jabberUser:
				...
				toolBoxInterface=self.interfaceGui.ui.toolBox
	...
	def connection(self, connected):
		...
		if connected==1:
			...
			if connected==-1:
				reply=QtGui.QMessageBox.warning(self, self.tr("Warning"), self.tr( "Could not authenticate!"))
			elif connected==-2:
				reply=QtGui.QMessageBox.warning(self, self.tr("Warning"),   self.tr("Could not connect!"))
